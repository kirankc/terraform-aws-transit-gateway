...
resource "aws_route_table" "vpc-1-rtb" {
  ...
  route {
    ...
    transit_gateway_id = "${aws_ec2_transit_gateway.tgw-partial-mesh.id}"
  }
  ...
  depends_on = ["aws_ec2_transit_gateway.tgw-partial-mesh"]
}
...
resource "aws_ec2_transit_gateway" "tgw-partial-mesh" {
...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw-att-vpc-1" {
  ...
  tags               = {
    Name             = "${var.scenario}-att-vpc1-dev"
    scenario         = "${var.scenario}"
  }
  depends_on = ["aws_ec2_transit_gateway.tgw-partial-mesh"]
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw-att-vpc-2" {
  ...
  transit_gateway_default_route_table_propagation = false
  tags               = {
    Name             = "${var.scenario}-att-vpc2-dev"
    scenario         = "${var.scenario}"
  }
  depends_on = ["aws_ec2_transit_gateway.tgw-partial-mesh"]
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw-att-vpc-3" {
  ...
  transit_gateway_default_route_table_propagation = false
  tags               = {
    Name             = "${var.scenario}-att-vpc3-shared"
    scenario         = "${var.scenario}"
  }
  depends_on = ["aws_ec2_transit_gateway.tgw-partial-mesh"]
}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw-att-vpc-4" {
  ...
  transit_gateway_default_route_table_propagation = false
  tags               = {
    Name             = "${var.scenario}-att-vpc4-prod"
    scenario         = "${var.scenario}"
  }
  depends_on = ["aws_ec2_transit_gateway.tgw-partial-mesh"]
}
...
resource "aws_customer_gateway" "cgw" {
  bgp_asn    = "${var.customergateway_bgp_asn}"
  ip_address = "${var.customergateway_ip}"
  type       = "ipsec.1"
  tags = {
    Name = "${var.scenario}-CGW"
  }
}

resource "aws_vpn_connection" "TGWVPN" {
  customer_gateway_id = aws_customer_gateway.cgw.id
  transit_gateway_id  = "${aws_ec2_transit_gateway.tgw-partial-mesh.id}"
  type                = aws_customer_gateway.cgw.type
  tags = {
    Name = "${var.scenario}-TGWVPN"
  }
  depends_on = ["aws_ec2_transit_gateway.tgw-partial-mesh", "aws_customer_gateway.cgw"]
}
...
resource "aws_key_pair" "tgw-partial-mesh-keypair" {
  key_name   = "tgw-partial-mesh-keypair"
...
resource "aws_security_group" "secgrp-vpc1-dev" {
  name        = "secgrp-vpc1-dev"
  description = "tgw-partial-mesh: Allow SSH and ICMP traffic"
  vpc_id      = "${aws_vpc.vpc-1.id}"
...
resource "aws_security_group" "secgrp-vpc2-dev" {
  name        = "secgrp-vpc2-dev"
  description = "tgw-partial-mesh: Allow SSH and ICMP traffic"
  vpc_id      = "${aws_vpc.vpc-2.id}"
...
resource "aws_security_group" "secgrp-vpc3-shared" {
  name        = "secgrp-vpc3-shared"
  description = "tgw-partial-mesh: Allow SSH and ICMP traffic"
  vpc_id      = "${aws_vpc.vpc-3.id}"
...
resource "aws_security_group" "secgrp-vpc4-prod" {
  name        = "secgrp-vpc4-prod"
  description = "tgw-partial-mesh: Allow SSH and ICMP traffic"
  vpc_id      = "${aws_vpc.vpc-4.id}"
...
resource "aws_instance" "tgw-partial-mesh-instance1-dev" {
  ami                         = "${data.aws_ami.ubuntu.id}"
  instance_type               = "t2.micro"
  subnet_id                   = "${aws_subnet.vpc-1-sub-a.id}"
  vpc_security_group_ids     = [ "${aws_security_group.secgrp-vpc1-dev.id}" ]
  key_name                    = "${aws_key_pair.tgw-partial-mesh-keypair.key_name}"
  private_ip                  = "10.10.1.10"

  tags = {
    Name = "tgw-partial-mesh-instance1-dev"
    scenario    = "${var.scenario}"
    env         = "dev"
    az          = "${var.az1}"
    vpc         = "1"
...
resource "aws_instance" "tgw-partial-mesh-instance2-dev" {
  ami                         = "${data.aws_ami.ubuntu.id}"
  instance_type               = "t2.micro"
  subnet_id                   = "${aws_subnet.vpc-2-sub-a.id}"
  vpc_security_group_ids     = [ "${aws_security_group.secgrp-vpc2-dev.id}" ]
  key_name                    = "${aws_key_pair.tgw-partial-mesh-keypair.key_name}"
  private_ip                  = "10.11.1.10"

  tags = {
    Name = "tgw-partial-mesh-instance2-dev"
    scenario    = "${var.scenario}"
    env         = "dev"
    az          = "${var.az1}"
    vpc         = "2"
...
resource "aws_instance" "tgw-partial-mesh-instance3-shared" {
  ami                         = "${data.aws_ami.ubuntu.id}"
  instance_type               = "t2.micro"
  subnet_id                   = "${aws_subnet.vpc-3-sub-a.id}"
  vpc_security_group_ids     = [ "${aws_security_group.secgrp-vpc3-shared.id}" ]
  key_name                    = "${aws_key_pair.tgw-partial-mesh-keypair.key_name}"
  private_ip                  = "10.12.1.10"
  associate_public_ip_address = true

  tags = {
    Name = "tgw-partial-mesh-instance3-shared"
    scenario    = "${var.scenario}"
    env         = "shared"
    az          = "${var.az1}"
    vpc         = "3"
...
resource "aws_instance" "tgw-partial-mesh-instance4-prod" {
  ami                         = "${data.aws_ami.ubuntu.id}"
  instance_type               = "t2.micro"
  subnet_id                   = "${aws_subnet.vpc-4-sub-a.id}"
  vpc_security_group_ids     = [ "${aws_security_group.secgrp-vpc4-prod.id}" ]
  key_name                    = "${aws_key_pair.tgw-partial-mesh-keypair.key_name}"
  private_ip                  = "10.13.1.10"

  tags = {
    Name = "tgw-partial-mesh-instance4-prod"
    scenario    = "${var.scenario}"
    env         = "prod"
    az          = "${var.az1}"
    vpc         = "4"
...
output "PUBLIC_IP" { value = "${aws_instance.tgw-partial-mesh-instance3-shared.public_ip}" }
